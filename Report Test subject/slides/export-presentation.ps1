...
try {
    ...
    Add-TextShape -Slide $slide -Left 58 -Top 168 -Width 540 -Height 86 -Text "Evidence-first defense deck aligned with the final submission baseline.`n$executedCases / $totalCases cases executed, $passCases pass, $failCases fail, $defectCount live tracked defects." -FontSize 16 -FontColor $colors.White -FontName 'Segoe UI' | Out-Null
    ...
    Add-SectionHeader -Slide $slide -StepLabel '01  Risk-Based Scope' -Title 'System Surfaces And High-Risk Areas' -Subtitle 'The scope is intentionally centered on business-critical flows that are most likely to break revenue, permissions, stock, or reporting.' -AccentColor $colors.Accent -TitleColor $colors.Ink -SubtitleColor $colors.Slate -SlideWidth $slideWidth
    $scopeCards = @(
    ...
    Add-Rectangle -Slide $slide -Left 0 -Top 0 -Width $slideWidth -Height $slideHeight -FillColor $colors.Surface | Out-Null
    Add-SectionHeader -Slide $slide -StepLabel '02  Strategy And Evidence Model' -Title 'How The Team Turned Design Into Defensible Evidence' -Subtitle 'The deck shows a compact strategy summary, then a visible trace from scenario to issue tracker.' -AccentColor $colors.Accent -TitleColor $colors.Ink -SubtitleColor $colors.Slate -SlideWidth $slideWidth
    Add-TextShape -Slide $slide -Left 48 -Top 150 -Width 288 -Height 186 -Text "Approach`n`n- source-based audit instead of a generic template`n- black-box execution with white-box-informed edge cases`n- manual UI validation plus API regression plus targeted UI automation" -FontSize 14 -FontColor $colors.Ink -FontName 'Segoe UI' -FillColor $colors.White -LineColor $colors.Border -Rounded -Margin 16 | Out-Null
    Add-TextShape -Slide $slide -Left 350 -Top 150 -Width 562 -Height 108 -Text "Evidence Chain`nEvery key claim in the report can be walked forward through design, execution, runtime proof, defect logging, and GitHub issue tracking." -FontSize 15 -FontColor $colors.Ink -FontName 'Segoe UI' -FillColor $colors.White -LineColor $colors.Border -Rounded -Margin 16 | Out-Null
    ...
    Add-TextShape -Slide $slide -Left 48 -Top 364 -Width 864 -Height 98 -Text "Defense point`nThe team does not rely on pass-fail labels alone. Every significant result is backed by a visible evidence file and, when failing, by a live GitHub issue with severity and priority." -FontSize 14 -FontColor $colors.Ink -FontName 'Segoe UI' -FillColor $colors.AccentSoft2 -LineColor $colors.Accent -Rounded -Margin 16 | Out-Null

    # Slide 4
    $slide = $presentation.Slides.Add(4, 12)
    Add-Rectangle -Slide $slide -Left 0 -Top 0 -Width $slideWidth -Height $slideHeight -FillColor $colors.Surface | Out-Null
    Add-SectionHeader -Slide $slide -StepLabel '03  Coverage Snapshot' -Title 'Coverage Is Complete, Ownership Is Traceable' -Subtitle 'The design baseline is wide enough for rubric coverage and explicit enough to prove that the UI workload was split without overlap.' -AccentColor $colors.Accent -TitleColor $colors.Ink -SubtitleColor $colors.Slate -SlideWidth $slideWidth
    ...
    Add-Rectangle -Slide $slide -Left 0 -Top 0 -Width $slideWidth -Height $slideHeight -FillColor $colors.DarkBg | Out-Null
    Add-SectionHeader -Slide $slide -StepLabel '04  Automation Implementation' -Title 'Automation Supports The Manual Baseline Instead Of Replacing It' -Subtitle 'The automation layer proves basic repeatability, visible tooling, and result traceability for both UI and API execution.' -AccentColor $colors.Teal -TitleColor $colors.White -SubtitleColor $colors.AccentSoft -SlideWidth $slideWidth
    ...
    Add-Rectangle -Slide $slide -Left 0 -Top 0 -Width $slideWidth -Height $slideHeight -FillColor $colors.Surface | Out-Null
    Add-SectionHeader -Slide $slide -StepLabel '06  Result Comparison' -Title 'Expected Result, Actual Result, Status, Then Evidence' -Subtitle 'This is the critical grading slide: the team defends each status through outcome comparison instead of relying on pass-fail labels alone.' -AccentColor $colors.Accent -TitleColor $colors.Ink -SubtitleColor $colors.Slate -SlideWidth $slideWidth
    ...
    Add-Rectangle -Slide $slide -Left 0 -Top 0 -Width $slideWidth -Height $slideHeight -FillColor $colors.Surface | Out-Null
    Add-SectionHeader -Slide $slide -StepLabel '07  Metrics Snapshot' -Title 'Execution Is Complete, Stability Is Not Yet Complete' -Subtitle 'The metric view is strong because it distinguishes coverage from product quality instead of merging them into a single conclusion.' -AccentColor $colors.Accent -TitleColor $colors.Ink -SubtitleColor $colors.Slate -SlideWidth $slideWidth
    ...
    Add-Rectangle -Slide $slide -Left 0 -Top 0 -Width $slideWidth -Height $slideHeight -FillColor $colors.Surface | Out-Null
    Add-SectionHeader -Slide $slide -StepLabel '08  Defect Management' -Title 'Defects Are Logged, Reproduced, And Linked To Live Issues' -Subtitle 'The submission now includes visible issue-tracker evidence with severity and priority, which is a direct grading requirement.' -AccentColor $colors.Accent -TitleColor $colors.Ink -SubtitleColor $colors.Slate -SlideWidth $slideWidth
    ...
    Add-TextShape -Slide $slide -Left 496 -Top 464 -Width 416 -Height 40 -Text 'All four confirmed defects were rerun on 2026-04-11 and still reproduced, so the current issue state remains open and justified.' -FontSize 12 -FontColor $colors.Slate -FontName 'Segoe UI' | Out-Null
    ...
    Add-TextShape -Slide $slide -Left 48 -Top 148 -Width 402 -Height 206 -Text "Proven today`n`n- strong traceability from scenario to issue tracker`n- 63 / 63 cases executed with 0 Not Run`n- automation demonstrates both pass and fail evidence`n- packaging is reviewable and evidence-first" -FontSize 15 -FontColor $colors.White -FontName 'Segoe UI' -FillColor $colors.DarkSurface -LineColor $colors.Success -Rounded -Margin 18 | Out-Null
